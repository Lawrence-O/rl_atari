...
from core.networks import DuelingDQNNetwork
from core.buffer import PrioritizedReplayBuffer


class NoNoiseRainbowAgent(Agent):
    def __init__(self, env, config, device="cpu"):
        ...
        """
        super(NoNoiseRainbowAgent, self).__init__(env, device)
        ...
        # Initialize networks
        self.policy_net = DuelingDQNNetwork(state_shape, num_actions, device).to(device)
        self.target_net = DuelingDQNNetwork(state_shape, num_actions, device).to(device)
    ...
    def train(self, experience=None):
        ...
        return {
            ...
            # For occasional network distance checking (every 1000 steps)
